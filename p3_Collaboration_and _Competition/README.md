...
and velocity of the ball and racket. Each agent receives its own,
local observation.
...
agents must get an average score of +0.5 (over 100 consecutive episodes,
...
```
  ...
  Alternatively, if you are using Windows 10 and experienced issues with `pip install .` ,
place the provided requirements.txt file in the `deep-reinforcement-learning` folder and run:
...
Instructions for training the agents are provided in `Tennis.ipynb`
